...
class IMAGENETDOGS(ImageDataset):
    """
    """
    base_folder = 'imagenetDogs'
    ...
    def __init__(self,
                 ...
                 fold=0,
                 **kwargs):
        super(IMAGENETDOGS, self).__init__(root, 
                                         part=part,
                                         val_size=val_size,
    ...
    def __getitem__(self, index):
        ...
        For non-constrained: we want to iterate over the whole data.
    ...
    def download(self):
        ...
        ### training 
        trainpath = os.path.join(self.dataset_path, 'traindogs')
        ...
        ### Test 
        valpath = os.path.join(self.dataset_path, 'valdogs')
...
# "ours" Data Augmentation Strategy from SCAN
normalize_imagenetDogs = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])

transforms_imagenetDogs_test = transforms.Compose([transforms.ToPILImage(),
                                                   transforms.CenterCrop(224),
                                                   transforms.ToTensor(), 
                                                   normalize_imagenetDogs]
                                                )

transforms_imagenetDogs_train = transforms.Compose([
...
transforms_imagenetDogs_weak = transforms.Compose([
                                               transforms.ToPILImage(),
                                               transforms.CenterCrop(224),
                                               transforms.RandomCrop(224),
                                               transforms.RandomHorizontalFlip(),
...
transforms_imagenetDogs_strong = transforms.Compose([transforms.ToPILImage(),
                                                transforms.RandomHorizontalFlip(),
                                                transforms.CenterCrop(224),
                                                transforms.RandomCrop(224),
                                                Augment(4),
